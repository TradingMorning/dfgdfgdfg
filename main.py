...
app = FastAPI(title="StreamVault Universal Engine", version="12.0.0")
...
# ==========================================
# 1. FRONTEND UI (AUTOMATED RESOLVER)
# ==========================================
...
<head>
    ...
    <title>StreamVault - Automated YouTube Downloader</title>
...
<body class="bg-[#080c14] text-gray-100 min-h-screen flex flex-col items-center justify-start p-4 sm:p-8">
    ...
    <header class="w-full max-w-3xl text-center my-8">
        <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-semibold mb-3">
            <i class="fa-solid fa-wand-magic-sparkles"></i> 100% Automated Extraction
        </div>
        <h1 class="text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-500">
            YouTube StreamVault
        </h1>
        <p class="text-gray-400 mt-2 text-sm">Paste any link to download MP4 video & audio directly</p>
    </header>
    ...
    <main class="w-full max-w-3xl space-y-6">
        ...
        <div id="loader" class="hidden my-8 text-center">
            ...
            <p class="text-gray-400 text-xs mt-2">Resolving high-speed video streams...</p>
        </div>
        ...
        <div id="detailsCard" class="hidden bg-[#121826] p-6 rounded-2xl border border-gray-800 shadow-2xl">
            <div class="flex flex-col md:flex-row gap-6">
                ...
                <div class="flex-grow flex flex-col justify-between">
                    ...
                    <div class="mt-6">
                        <label class="text-xs font-bold uppercase tracking-wider text-gray-400 mb-2 block">Choose Quality to Download:</label>
    ...
    <script>
        function extractId(url) {
            const m = url.match(/(?:youtu\\.be\\/|v\\/|u\\/\\w\\/|embed\\/|watch\\?v=|shorts\\/)([^#\\&\\?\\s]{11})/);
            return m ? m[1] : null;
        }
        ...
        async function processVideo() {
            ...
            try {
                // 1. Fetch metadata (Title, Thumbnail) via server
                const metaResp = await fetch(`/api/info?url=${encodeURIComponent(urlInput)}`);
                const meta = await metaResp.json();

                if (!metaResp.ok || meta.status !== "success") {
                    throw new Error(meta.detail || "Unable to load video info.");
                }

                document.getElementById("videoTitle").innerText = meta.data.title;
                document.getElementById("videoThumb").src = meta.data.thumbnail;
                document.getElementById("videoUploader").innerText = meta.data.uploader;

                // 2. Build automated direct stream formats
                const qContainer = document.getElementById("qualityButtons");
                qContainer.innerHTML = "";

                const formats = [
                    { label: "1080p Full HD (MP4)", quality: "1080", isAudio: false, icon: "fa-circle-play text-emerald-400" },
                    { label: "720p HD (MP4)", quality: "720", isAudio: false, icon: "fa-video text-indigo-400" },
                    { label: "360p Medium (MP4)", quality: "360", isAudio: false, icon: "fa-video text-blue-400" },
                    { label: "Audio (MP3)", quality: "mp3", isAudio: true, icon: "fa-music text-pink-400" }
                ];

                formats.forEach(f => {
                    const btn = document.createElement("button");
                    btn.className = "px-3.5 py-2 rounded-lg bg-[#1a2337] hover:bg-indigo-600/40 border border-gray-700 hover:border-indigo-500 text-xs font-medium text-white flex items-center gap-2 transition";
                    btn.innerHTML = `<i class="fa-solid ${f.icon}"></i> ${f.label}`;
                    btn.onclick = () => downloadDirect(videoId, f.quality, f.isAudio, btn);
        ...
        async function downloadDirect(videoId, quality, isAudio, btn) {
            const originalText = btn.innerHTML;
            btn.innerHTML = `<i class="fa-solid fa-spinner animate-spin"></i> Getting link...`;
            btn.disabled = true;

            const ytUrl = `https://www.youtube.com/watch?v=${videoId}`;
            
            // Cobalt & Invidious automated fallback list
            const endpoints = [
                { url: "https://cobalt-api.kwiatekm.pl/", mode: "cobalt" },
                { url: "https://api.cobalt.tools/", mode: "cobalt" }
            ];
            ...
            if (downloadUrl) {
                ...
                alert("Stream servers are currently busy. Please try another quality.");
            }
