...
app = FastAPI(title="StreamVault - Universal YouTube Engine", version="18.0.0")
...
# ==========================================
# 1. UI WITH CLIENT-DIRECT RESOLUTION
# ==========================================
...
<head>
    ...
    <title>StreamVault - Direct YouTube Downloader</title>
...
<body class="bg-[#080c14] text-gray-100 min-h-screen flex flex-col items-center justify-start p-4 sm:p-8">
    ...
    <header class="w-full max-w-3xl text-center my-8">
        <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-semibold mb-3">
            <i class="fa-solid fa-shield-halved"></i> Client-Direct Tunneling Active
        </div>
        <h1 class="text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-500">
            YouTube StreamVault
        </h1>
        <p class="text-gray-400 mt-2 text-sm">Download 1080p, 720p, 360p & MP3 without IP or Bot blocks</p>
    </header>
    ...
    <main class="w-full max-w-3xl space-y-6">
        ...
        <div id="loader" class="hidden my-8 text-center">
            ...
            <p class="text-gray-400 text-xs mt-2">Resolving streams via client connection...</p>
        </div>
    ...
    <script>
        ...
        async function processVideo() {
            ...
            try {
                ...
                if (!metaResp.ok || meta.status !== "success") {
                    throw new Error(meta.detail || "Unable to resolve video information.");
                }
                ...
                formats.forEach(f => {
                    ...
                    btn.onclick = () => directDownload(videoId, f.q, f.isAudio, btn);
        ...
        async function directDownload(videoId, quality, isAudio, btn) {
            const originalText = btn.innerHTML;
            btn.innerHTML = `<i class="fa-solid fa-spinner animate-spin"></i> Fetching stream...`;
            btn.disabled = true;

            const ytUrl = `https://www.youtube.com/watch?v=${videoId}`;
            
            // Public Dedicated Proxy Resolver Nodes
            const instances = [
            ...
            if (streamUrl) {
                ...
                alert("Download server is momentarily busy. Please try another quality button.");
            }
...
# ==========================================
# 2. METADATA API (OFFICIAL OEMBED - ZERO BOT CHECKS)
# ==========================================
@app.get("/api/info")
